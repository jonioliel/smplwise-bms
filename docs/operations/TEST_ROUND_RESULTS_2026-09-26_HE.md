...
כל שורה. בדיקה נכשלת קודם (`test_a_merge_restore_counts_only_the_rows_it_added`), התיקון, והבדיקה ירוקה. commit 35e26c4.
...
| 35e26c4 | fix(backup): השחזור מדווח את השורות שכתב (סעיף 15) |
| 176478a | test(crop): ייבוא מחדש בחיתוך אחר (סעיף 19) |
| 96aad47 | test(plan-studio): 7א, 10, 26, 30, 44–48, 54, 55, 66 + gltf-validator |
| 39aca33 | test(form-round9): המפרט החדש לסעיפים בלי בדיקה משלהם |
| 4e5710d | test(event-page): עמוד אירוע על מופע זמני (סעיפים 11, 26) |
| 173fea2 | test(round9): תיקוני ביקורת — המתנות חיוביות במקום השהיות, בדיקת קובץ ה־SQLite של המופע הזמני, מונה השניות, ההרחבה הנדרשת ב־glTF, נעילת גרסת gltf-validator |
