...
| T5 | Legacy display-offset feature (auto/manual/auto+manual) exists because of T1/T2. | code + owner | Replace with TimeAdapter; import the owner's setting only to compare during parity tests. |
...
| S6 | `/ISAPI/ContentMgmt/download` accepted body/method varies; legacy tries 4 XML variants × GET/POST. | code (owner: download works) | Pin the working variant in T006 and stop probing at runtime. |
