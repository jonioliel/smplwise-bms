...
const SCREENS: { mode: string; items: Entry[] }[] = [
  {
    mode: 'סקירה ומצלמות',
    items: [
      { sc: 'SC01', name: 'סקירה (Dashboard)', route: '#/live', phase: 'PILOT', board: '1:01 · 2:15' },
      { sc: 'SC07', name: 'כל המצלמות (Grid)', route: '#/live/wall', phase: 'PILOT', board: '1:06' },
      { sc: 'SC08', name: 'מצלמה בודדת', route: '#/live/cameras/cam-4', phase: 'PILOT', board: '1:05' },
      { sc: 'SC09', name: 'תצוגות שמורות', route: '#/live/views', phase: 'PILOT', board: 'legacy' },
      { sc: 'SC27', name: 'בריאות מצלמות', route: '#/system/devices', phase: 'PILOT', board: '2:12' },
  ...
  },
  {
    ...
    items: [
      ...
      { sc: 'SC19', name: 'חיפוש AI', route: '#/investigate/search', phase: 'BETA', board: '2:09' },
      { sc: 'SC21', name: 'התראות וחוקים', route: '#/investigate/rules', phase: 'BETA', board: '3:19' },
  ...
  },
  {
    mode: 'הגדרות',
    items: [
      { sc: 'SC28', name: 'הגדרות המערכת', route: '#/system/diagnostics', phase: 'PILOT', board: '3:23' },
        ...
        <div class="groups">
          ${SCREENS.map(
            (g) => html`<sw-card heading=${g.mode}>
              ${g.items.map((it) => html`<a href=${it.route}><span class="sc">${it.sc}</span><span class="name">${it.name}</span><span class="board">${it.board}</span><sw-badge kind=${PHASE_KIND[it.phase]} label=${it.phase}></sw-badge><sw-icon name="chevron" size=${12}></sw-icon></a>`)}
            </sw-card>`,
          )}
        </div>
      </sw-page>
